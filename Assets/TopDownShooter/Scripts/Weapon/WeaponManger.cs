...
public class WeaponManger : MonoBehaviour
{
    ...
    [Header("Hospital")]
    public bool withinHos;
    public GameObject hosBTN;
    public LayerMask hosLayer;
    ...
    void Update()
    {
        if(!player.inCar && !player.inHelicopter)
        {
            ...
            withinHos = Physics.CheckSphere(transform.position, radius, hosLayer);
            ...
            if (withinHos)
            {
                hosBTN.SetActive(true);
            }
            else
            {
                hosBTN.SetActive(false);
            }
