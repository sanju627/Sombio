...
public class Zombie : MonoBehaviour
{
    ...
    public int minSpawmnZombieCount;
    public int maxSpawmnZombieCount;
    ...
    [Header("Scream")]
    public float screamCheckRadius;
    ...
    public GameObject PukeProjectile;
    ...
    public void JumpUpdate()
    {
        ...
        if(isLaunching)
        {
            agent.baseOffset += Time.deltaTime * jumpSpeed;
        }
    }
    ...
    void SearchWalkPoint()
    {
        ...
        else if(houseZombie || Explode || Jump)
        {
    ...
    public void Attack()
    {
        ...
        Collider[] animalCol = Physics.OverlapSphere(attackPoint.position, radius, animalLayer);
        ...
        foreach (Collider a in animalCol)
        {
            a.GetComponent<Animal>().TakeDamage(damage);
        }
    }
    ...
    public void Explode_VFX()
    {
        Instantiate(ExplosionVFX, transform.position, transform.rotation);

        player.GetComponent<AudioSource>().PlayOneShot(explosionSFX);
    ...
    public void Shoot()
    {
        GameObject puke = Instantiate(PukeProjectile, attackPoint.position, attackPoint.rotation);
        Rigidbody rb = puke.GetComponent<Rigidbody>();
        rb.AddForce(attackPoint.forward * throwForce);
    }

    public void Scream_Call()
    {
        Collider[] zombiesCol = Physics.OverlapSphere(attackPoint.position, screamCheckRadius, zombiesLayer);

        audio.PlayOneShot(growlingSFX[Random.Range(0, growlingSFX.Length)]);

        /*foreach (var zombie in zombiesCol)
        {
            zombie.GetComponent<Zombie>().withinTarget = true;
            zombie.GetComponent<Zombie>().target = player.GetComponent<ZombiesTarget>();
        }*/

        player = GameObject.FindGameObjectWithTag("Player").GetComponent<Player>();

        Transform tSpawn = player.z_spawnPos[Random.Range(0, player.z_spawnPos.Length)];        

        int spawns = Random.Range(minSpawmnZombieCount, maxSpawmnZombieCount);
    ...
    {
        isLaunching = true;
        player.OnZombie = true;
    }

    public void JumpOnPlayer()
    {
        player.EnableOnZombieVFX();
        player.OnZombieTriggered();
    }
